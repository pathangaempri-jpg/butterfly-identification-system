...
export default function ObservationsPage() {
  ...
  const verifyMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/observations/${id}/verify`),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/observations/${id}/reject`),
    onSuccess: () => {
  ...
  const columns: Column<Observation>[] = [
      ...
      cell: (o) => (
        <div>
          <p className="text-sm font-medium">
            {o.title ?? o.species?.common_name ?? "Unidentified"}
          </p>
          <p className="text-xs text-muted-foreground">
            by @{o.user?.username ?? "unknown"} · {o.state?.name ?? `State ${o.state_id}`}
          </p>
        </div>
      ),
    },
    {
      ...
      cell: (o) => o.species ? (
        <div>
          <p className="text-xs font-medium">{o.species.common_name}</p>
          <p className="text-[10px] text-muted-foreground italic">{o.species.scientific_name}</p>
        </div>
    ...
    },
    {
      key: "privacy",
      header: "Privacy",
      cell: (o) => <Badge variant="outline" className="text-xs capitalize">{o.privacy}</Badge>,
    },
    {
      key: "date",
      header: "Submitted",
