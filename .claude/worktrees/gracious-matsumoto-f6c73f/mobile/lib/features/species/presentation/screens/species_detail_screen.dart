          ...
          Expanded(
            ...
            child: Stack(
              children: [
                ...
                const Positioned(top: 8, left: 8, child: _CircleBack()),
              ],
            ),
          ),
          Expanded(
...
List<Widget> _content(BuildContext context, WidgetRef ref, SpeciesDetail d) {
  return [
    ...
    if (d.hasDistribution) ...[
      const _SectionTitle('Distribution in India'),
      Wrap(
        spacing: SpaceTokens.sm,
        runSpacing: SpaceTokens.sm,
        children: d.states
            .map((s) => _Chip(label: s, icon: Icons.place_outlined))
            .toList(),
      ),
      const SizedBox(height: SpaceTokens.sm),
      Text(
        'Interactive distribution map arrives in Phase 7.',
