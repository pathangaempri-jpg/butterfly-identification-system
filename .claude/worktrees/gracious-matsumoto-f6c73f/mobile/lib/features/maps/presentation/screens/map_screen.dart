...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  void _recenter() {
    ...
    setState(() => _zoom = _indiaZoom);
  }

  void _openFilter() {
    final states = ref.read(statesProvider).valueOrNull ?? const <IndiaState>[];
    final currentState = ref.read(mapStateFilterProvider);
    ...
    AppBottomSheet.show(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(),
          const SizedBox(height: SpaceTokens.md),
          Text('State', style: TypographyTokens.textTheme.titleSmall),
          const SizedBox(height: SpaceTokens.xs),
          Wrap(
            spacing: SpaceTokens.sm,
            runSpacing: SpaceTokens.sm,
            children: [
              ChoiceChip(
                ...
                onSelected: (_) {
                  ref.read(mapStateFilterProvider.notifier).state = null;
                  Navigator.of(context).pop();
                },
              ),
              ...states.map((s) => ChoiceChip(
                    label: Text(s.name),
                    selected: currentState == s.id,
                    onSelected: (_) {
                      ref.read(mapStateFilterProvider.notifier).state = s.id;
                      Navigator.of(context).pop();
                    },
                  )),
            ],
        ...
        children: [
          ...
          if (sightingsAsync.isLoading)
            ...
            const Positioned(
              ...
              right: 0,
              bottom: 120,
              child: _NoSightingsBanner(),
            ),

          // ── Floating controls ────────────────────────────────────────────
          Positioned(
            right: SpaceTokens.base,
            bottom: SpaceTokens.xxl + 80,
            child: Column(
              children: [
                _GlassIconButton(icon: Icons.my_location, onTap: _goToMyLocation),
                const SizedBox(height: SpaceTokens.sm),
                _GlassIconButton(
                    icon: Icons.public, onTap: _recenter),
              ],
            ),
          ),
        ],
