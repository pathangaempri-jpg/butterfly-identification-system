...
def _obs_to_dict(row: dict, include_user=False, include_images=True,
                 current_user_id=None, social=None) -> dict:
...
def _serialize_page(rows: list, current_user_id=None, include_user=True) -> list:
...
def get_observation(obs_id: str, requesting_user_id: str) -> dict:
    ...
    if obs["user_id"] != requesting_user_id:
        if obs.get("privacy") == "private" and not _is_staff(requesting_user_id):
...
def delete_observation(obs_id: str, user_id: str) -> None:
    sb = get_supabase()
    obs = _fetch_obs(obs_id)
    if not obs:
        raise ObservationError("Observation not found.", 404)
...
def admin_list_observations(page: int, per_page: int, filters: dict) -> tuple:
    ...
    items = _serialize_page(res.data, include_user=True)
    return items, res.count or 0
